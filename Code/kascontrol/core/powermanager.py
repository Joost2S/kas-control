...
class PowerManager(object):
	...
	def addRequest(self, objectUUID, duration=None):
		...
		with self.__lock:
			...
			r = Request(requestID, objectUUID, duration, priority)
	...
	def cancelRequest(self, requestID):
		...
		req = self.__getRequestFromID(requestID)
		if req is False:
			return False
		req.cancel()
		report = req.report()
		...
		self.__reports[requestID] = report
		return(report)
	...
	def __getRequestFromID(self, requestID):
	...
	def getReport(self, requestID):

		try:
			return(self.__reports[requestID])
		except KeyError:
			return(False)
...
class Request(object):
	...
	def __assignDependant(self, obj):
