...
# global variables
speriod=(15*60)-1
dbname='/home/j2s/greenhousefiles/dbfiles/datalog2016.db'
...
def get_data(interval):
	...
	if interval == None:
		...
		curs.execute("SELECT * FROM data WHERE timestamp>" + str(int(time.time())-int(interval)*60*60))
...
def create_table(rows):
	...
	for row in rows[:-1]:
		rowstr="\t\t\t\t\t['{0}', {1}, {2}, {3}, {4}, {5}, {6}, {7}],\n".format(str(datetime.fromtimestamp(int(row[0])).strftime('%Y-%m-%d %H:%M:%S')),str(row[1]),perc(row[2]),perc(row[3]),perc(row[4]),perc(row[5]),perc(row[6]),perc(row[7]))
		chart_table+=rowstr
...
def perc(number):
	return(str(round((number / 40.95), 1)))
		...
		<script type="text/javascript">
			...
			function drawChart() {
				var data = google.visualization.arrayToDataTable([
					['Time', 'Temperature', 'light', 'mst1', 'mst2', 'mst3', 'mst4', 'mst5'],
%s
				]);
...
def show_graph():
	...
	print('\t\t\t<div id="curve_chart" style="width: 2552px; height: 1140px;"></div>')
...
def show_stats(option):
	...
	curs.execute("SELECT timestamp,max(temps) FROM moisture WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#	curs.execute("SELECT timestamp,max(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
	...
	curs.execute("SELECT timestamp,min(temps) FROM moisture WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#	curs.execute("SELECT timestamp,min(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
	...
	curs.execute("SELECT avg(temps) FROM moisture WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
...
def main():
	...
	printHTMLHead("Kas Control. Data weergave.", table)
