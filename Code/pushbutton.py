...
class pumpToggle(object):
	...
	def run(self):
		"""Toggles pump for given channel."""

		if (gs.curChan >= 0):
			if (gs.getPinDev(gs.hwc.valves[gs.curChan].led).getPinState(gs.getPinNr(gs.hwc.valves[gs.curChan].led))):
				gs.hwc.endRequest(gs.curChan)
			else:
				gs.hwc.requestPumping(gs.curChan)
				print("start pumping: ", gs.curChan)
